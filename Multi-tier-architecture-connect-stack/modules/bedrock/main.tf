...
resource "aws_bedrockagent_agent" "banking_assistant" {
  ...
  # Note: prompt_override_configuration with inference_configuration and prompt_state
  # is incompatible with DEFAULT prompt_creation_mode. Removed to fix validation error.

  tags = var.tags
}

# Wait for agent to be ready before creating aliases
